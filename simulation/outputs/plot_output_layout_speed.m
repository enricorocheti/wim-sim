...
%         for i = 1:length(sensor_counts)
%             df = axle_data{i};
%             mae_values(i) = df.mae_mv(df.speed == selected_speed);
%         end
%         p = plot(sensor_counts, mae_values, markers{dist_idx}, 'DisplayName', sprintf('d = %s', strrep(sensor_dist, '_', '.')));
%         p.MarkerFaceColor = p.Color;
%     end
%     legend;
%     hold off;
%     saveas(gcf, sprintf('figures_layout/axle_mae_v%s.png', num2str(selected_speed)));
% 
%     %% GVW MAE
%     figure('Position', [fig_x, fig_y, fig_w, fig_h]);
%     hold on;
...
%     legend;
%     hold off;
%     saveas(gcf, sprintf('figures_layout/gvw_mae_v%s.png', num2str(selected_speed)));
end
